...
const StepIcon: React.FC<{ icon: React.ReactNode, bgColor: string, textColor: string }> = ({ icon, bgColor, textColor }) => (
  <div className={`w-16 h-16 ${bgColor} ${textColor} rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm`}>
    {icon}
  </div>
);
...
export const ForgotPassword: React.FC<ForgotPasswordProps> = ({ onNavigate }) => {
  ...
  const handleSendCode = () => {
    if (!email.includes('@')) {
      setForgotError('נא להזין כתובת אימייל תקינה');
      return;
    }
    setForgotError('');
    setCode(['', '', '', '']);
  ...
  const handleVerifyCode = () => {
    ...
    if (enteredCode === '1234') {
      ...
      setForgotError('הקוד שגוי. נסה שוב (הקוד להדגמה הוא 1234)');
    }
  };

  const handleResetPassword = () => {
    if (newPassword.length < 8) {
      setForgotError('הסיסמה חייבת להכיל לפחות 8 תווים');
      return;
    }
    if (newPassword !== confirmPassword) {
      setForgotError('הסיסמאות אינן תואמות');
      return;
    }
    ...
    if (newPassword === '88888888') {
      setNotification({
        type: 'error',
        message: 'אירעה שגיאת מערכת בעת עדכון הסיסמה. נא לנסות שוב מאוחר יותר.'
      });
      return;
    }
      ...
      case 'EMAIL':
        return (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <ScreenHeader 
              title="איפוס סיסמה" 
              subtitle="הזן את המייל שלך ונשלח לך קוד אימות בן 4 ספרות"
      ...
      case 'VERIFY':
        return (
          <div className="animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            <ScreenHeader 
              title="אימות חשבון" 
              subtitle={`הזן את הקוד ששלחנו לכתובת: ${email}`}
              className="text-center mb-8"
            />
            ...
            <div 
              ...
              dir="ltr"
              role="group"
              aria-label="קוד אימות בן 4 ספרות"
            >
              {code.map((digit, idx) => (
                <input
                  key={idx}
                  ...
                  className="w-14 h-16 text-center text-2xl font-black border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all"
                />
              ))}
            </div>
            ...
            <button 
              ...
              className="mt-6 text-slate-400 font-bold text-sm hover:text-blue-600 transition-colors"
            >
              שלח קוד מחדש
            </button>
          </div>
      ...
      case 'RESET':
        return (
          <div className="animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            <ScreenHeader 
              title="סיסמה חדשה" 
              subtitle="החשבון אומת בהצלחה. כעת ניתן לבחור סיסמה חדשה."
              className="text-center mb-8"
            />
