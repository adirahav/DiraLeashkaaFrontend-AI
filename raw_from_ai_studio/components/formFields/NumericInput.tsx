...
import { formatInputNumber } from '../../services/utils';
...
export const NumericInput: React.FC<NumericInputProps> = ({
  ...
  return (
    <div className={`flex flex-col gap-1.5 w-full text-right ${disabled ? 'opacity-60 grayscale-[0.2]' : ''}`}>
      ...
      />
      {error && (
        <span className="text-xs font-bold text-red-500 mt-0.5 animate-in fade-in slide-in-from-top-1 duration-200 mr-1">
          {error}
        </span>
      )}
    </div>
