      ...
      <nav className="bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm" dir="rtl">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between relative">
          <div className="flex items-center gap-4">
            {/* Back Button - Visible only on mobile for internal pages */}
            {isInternalPage && (
              <button 
            ...
            {/* Logo - Always centered on mobile, at the start on desktop */}
            <button 
              onClick={() => onNavigate('HOME')} 
          ...
          <ul className="flex items-center gap-4 list-none p-0 m-0">
            <li className="hidden sm:block text-sm font-bold text-slate-700">
              שלום, {userName}
            </li>
              ...
              <ul className="grid grid-cols-2 gap-2 px-2 list-none p-0 m-0">
                ...
                <li>
                  <Button 
                    variant="outline"
                    onClick={() => {
                      onNavigate('ACCESSIBILITY_STATEMENT');
                      setIsMenuOpen(false);
                    }}
                    className="!border-none !shadow-none !text-slate-500 hover:!bg-slate-50 !rounded-lg !text-sm !px-3 !py-2 !justify-start"
                    icon={Accessibility}
                ...
                <li>
                  <Button 
                    ...
                    className="!border-none !shadow-none !text-red-500 hover:!bg-red-50 !rounded-lg !text-sm !px-3 !py-2 !justify-start"
                    icon={LogOut}
                    iconSize={16}
                  >
                    התנתקות
