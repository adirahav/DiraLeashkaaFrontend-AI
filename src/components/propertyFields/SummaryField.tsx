...
const VARIANT_MAP: Record<SummaryFieldVariant, { bg: string; border: string; text: string; label: string }> = {
...
export const SummaryField: React.FC<SummaryFieldProps> = ({
  ...
  const v = VARIANT_MAP[variant] ?? VARIANT_MAP.slate;

  return (
    <div className={cn("flex flex-col gap-1.5 w-full text-right", className)}>
      <span
        className={cn(
          "text-base font-bold h-5 flex items-center",
          align === 'center' ? "mx-auto" : align === 'right' ? "mr-1" : "ml-1",
          labelClassName || v.label
        )}
      >
        {label}
      </span>
      ...
      >
        <span
          className={cn(
            "text-lg font-black",
            align === 'center' ? v.text : "text-black w-full text-right"
          )}
        >
          {value}
        </span>
